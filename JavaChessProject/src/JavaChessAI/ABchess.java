...
public class ABchess extends UserInterface {
	static String chessBoard[][]={
	    ...
	    public static String posibleK(int i) {
	        ...
	        for (int j=0; j<8; j++) {
	            int newRow = r + rowOffsets[j];
	            int newCol = c + colOffsets[j];
	            if (newRow < 0 || newRow >= 8 || newCol < 0 || newCol >= 8) {
	                continue; // Move is off the board
	            }
	            if (Character.isLowerCase(chessBoard[newRow][newCol].charAt(0)) || " ".equals(chessBoard[newRow][newCol])) {
